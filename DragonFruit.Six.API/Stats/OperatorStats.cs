...
namespace DragonFruit.Six.API.Stats
{
    public class Operator
    {
        ...
        /// <summary>
        /// The subtitle, as seen underneath the operator's name in game (from
        /// datasheet)
        /// </summary>
        ...
        public static async Task<IEnumerable<Operator>> GetOperatorStats(
            AccountInfo account, IEnumerable<Operator> data,
            string token) => (await GetOperatorStats(new[] { account }, data, token)
                                  .ConfigureAwait(false))
                                 .First();

        public static async Task<IEnumerable<Operator>> GetOperatorStats(
            AccountInfo account, Dictionary<string, string> operatorNameIndex,
            string token) => (await GetOperatorStats(new[] { account },
                                                     operatorNameIndex, token)
                                  .ConfigureAwait(false))
                                 .First();

        public static async Task<IEnumerable<IEnumerable<Operator>>>
        GetOperatorStats(IEnumerable<AccountInfo> accounts,
                         Dictionary<string, string> operatorNameIndex,
                         string token)
        {
            var data = operatorNameIndex.Select(
                x => new Operator { Name = x.Value, Index = x.Key });

            return await GetOperatorStats(accounts, data, token)
                .ConfigureAwait(false);
        }

        public static async Task<IEnumerable<IEnumerable<Operator>>>
        GetOperatorStats(IEnumerable<AccountInfo> accounts,
                         IEnumerable<Operator> data, string token)
        {
        ...
        public static async IAsyncEnumerable<IEnumerable<Operator>>
        GetOperatorStatsAsync(IEnumerable<AccountInfo> accounts,
                              IEnumerable<Operator> data, string token)
        {
            ...
            var baseKeys =
                "operatorpvp_kills,operatorpvp_headshot,operatorpvp_dbno,operatorpvp_death,operatorpvp_roundlost,operatorpvp_roundplayed,operatorpvp_roundwlratio,operatorpvp_roundwon,operatorpvp_timeplayed"
                    .Split(',');
            var operatorKeys =
                data.Where(x => !string.IsNullOrEmpty(x.OperatorActionId))
                    .Select(x => x.OperatorActionId);

            foreach (var group in filteredGroups)
            {
                var ids = group.Select(x => x.Guid);
                var uri = d6WebRequest.FormStatsUrl(
                    group.Key, ids, string.Join(',', operatorKeys.Concat(baseKeys)));

                var rawData =
                    await Task.Run(() => d6WebRequest.GetWebObject(uri, token));

                foreach (var x in ids) yield return rawData.ToOperatorStats(x, data);
            }
        }

        public static IEnumerable<Operator> GetOperators(string file)
        {
            var data = Uri.TryCreate(file, UriKind.Absolute, out _)
                           ? WebServices.StreamObject<JArray>(file)
                           : FileServices.ReadFile<JArray>(file);
