...
namespace DragonFruit.Six.API.Processing {
  public static class Alignments {
    public static IEnumerable<AccountInfo>ToAccounts(this JObject jObject) {
      ...
      foreach(var jToken in json) {
        ...
        yield return new AccountInfo{
            ...
            Platform = PlatformParser.PlatformEnumFor(
                item.GetString(Accounts.Platform)),
    ...
    public static GeneralStats ToGeneralStats(this JObject jObject,
                                              string guid) {
      if (jObject == null) {
        return new GeneralStats();
      }

      var json = (JObject) jObject [Misc.Results]
      [guid];
      ...
      return new GeneralStats{
          ...
          CasualKd = json.GetFloat(GeneralCasual.Kills, 1) /
                     json.GetFloat(GeneralCasual.Deaths, 1),

          CasualWins = json.GetUInt(GeneralCasual.Wins),
          CasualLosses = json.GetUInt(GeneralCasual.Losses),
          CasualWl = json.GetFloat(GeneralCasual.Wins, 1) /
                     json.GetFloat(GeneralCasual.Losses, 1),
          ...
          HuntKd = json.GetFloat(GeneralPvE.Kills, 1) /
                   json.GetFloat(GeneralPvE.Deaths, 1),

          HuntWins = json.GetUInt(GeneralPvE.Wins),
          HuntLosses = json.GetUInt(GeneralPvE.Losses),
          HuntWl = json.GetFloat(GeneralPvE.Wins, 1) /
                   json.GetFloat(GeneralPvE.Losses, 1),
          ...
          // Time Played
          TimePlayedCasual =
              TimeSpan.FromSeconds(json.GetDouble(GeneralCasual.Time)),
          TimePlayedTHunt =
              TimeSpan.FromSeconds(json.GetDouble(GeneralPvE.Time)),
          TimePlayedRanked =
              TimeSpan.FromSeconds(json.GetDouble(GeneralRanked.Time)),
          TimePlayedGeneral =
              TimeSpan.FromSeconds(json.GetDouble(OverallPvP.Time)),
          TimePlayedCustom =
              TimeSpan.FromSeconds(json.GetDouble(GeneralCustomGame.Time)),

          // Ranked
          RankedWins = json.GetUInt(GeneralRanked.Wins),
          RankedLosses = json.GetUInt(GeneralRanked.Losses),
          RankedWl = json.GetFloat(GeneralRanked.Wins, 1) /
                     json.GetFloat(GeneralRanked.Losses, 1),
          ...
          RankedKd = json.GetFloat(GeneralRanked.Kills, 1) /
                     json.GetFloat(GeneralRanked.Deaths, 1),
    ...
    public static Season ToSeason(this JObject jObject, string guid) {
      var json = (JObject) jObject [Misc.Players]
      [guid];

      return new Season{Guid = guid,
    ...
    ToOperatorStats(this JObject jObject, string guid,
                    IEnumerable<Operator>data) {
      var json = (JObject) jObject [Misc.Results]
      [guid];
      ...
      foreach(var op in data) {
        ...
        op.Deaths =
            json.GetUInt(string.Format(OverallOperator.Deaths, op.Index));
        op.Wins = json.GetUInt(string.Format(OverallOperator.Wins, op.Index));
        op.Losses =
            json.GetUInt(string.Format(OverallOperator.Losses, op.Index));
        op.Headshots =
            json.GetUInt(string.Format(OverallOperator.Headshots, op.Index));
        op.Downs = json.GetUInt(string.Format(OverallOperator.Downs, op.Index));
        op.RoundsPlayed =
            json.GetUInt(string.Format(OverallOperator.Rounds, op.Index));
        op.KD =
            json.GetFloat(string.Format(OverallOperator.Kills, op.Index), 1) /
            json.GetFloat(string.Format(OverallOperator.Deaths, op.Index), 1);
        op.WL =
            json.GetFloat(string.Format(OverallOperator.Wins, op.Index), 1) /
            json.GetFloat(string.Format(OverallOperator.Losses, op.Index), 1);

        op.Duration = TimeSpan.FromSeconds(
            json.GetUInt(string.Format(OverallOperator.Time, op.Index)));
    ...
    public static IEnumerable<WeaponStats>ToWeaponStats(this JObject jObject,
                                                        string guid) {
      var json = (JObject) jObject [Misc.Results]
      [guid];
      ...
      foreach(var index in References.WeaponClasses.Keys) {
        yield return new WeaponStats{
            ...
            Headshots =
                json.GetUInt(string.Format(OverallWeapon.Headshots, index)),
            ShotsFired =
                json.GetUInt(string.Format(OverallWeapon.ShotsFired, index)),
            ShotsLanded =
                json.GetUInt(string.Format(OverallWeapon.ShotsHit, index))};
      }
    }
  }
}
